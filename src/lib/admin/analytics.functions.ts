...
export const getTopSegments = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }): Promise<TopSegment[]> => {
    ...
    const { data: rows, error } = await (supabaseAdmin.rpc as any)(
      "analytics_top_segments",
      { p_days: effectiveDays(data.days), p_min_leads: 20 },
    );
...
export const getCohortWeekly = createServerFn({ method: "GET" })
  ...
  .inputValidator((input) =>
    z.object({ weeks: z.number().int().min(1).max(52).default(12) }).parse(input),
  )
