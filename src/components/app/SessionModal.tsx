...
import { Moon, Sun, X, Headphones, ShoppingCart, Lock } from "lucide-react";
...
export function SessionModal({
  session, dayTheme, done, onClose, onToggle, audio, checkoutUrl,
}: {
  session: PlanSession;
  dayTheme: string;
  done: boolean;
          ...
          {audio && audio.audio_url ? (
            <div className="mt-5 rounded-2xl border border-[color:var(--gold-warm)]/40 bg-white/70 p-4">
              <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-[color:var(--gold-warm)]">
                <Headphones className="h-3.5 w-3.5" /> Áudio guiado · {audio.title}
              </div>
              <audio src={audio.audio_url} controls preload="none" className="mt-2 w-full" />
            </div>
          ) : checkoutUrl ? (
            <a
              href={checkoutUrl}
